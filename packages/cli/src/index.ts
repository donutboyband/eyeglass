...
function setupCopilotConfig(dryRun: boolean): boolean {
  const cwd = process.cwd();
...
function setupCodexConfig(dryRun: boolean): boolean {
  const cwd = process.cwd();
...
function setupNextAppRouter(cwd: string, dryRun: boolean): boolean {
...
`;
  ...
  newContent = newContent.replace(
    /(\{[\s]*children[\s]*\})/g,
    '<EyeglassProvider>$1</EyeglassProvider>'
  );
...
function setupNextPagesRouter(cwd: string, dryRun: boolean): boolean {
...
function printUsageInstructions(agents: AgentType[]): void {
...
// Parse agent flags
const agentFlags: AgentType[] = [];
if (flags.includes('--claude')) agentFlags.push('claude');
if (flags.includes('--copilot')) agentFlags.push('copilot');
if (flags.includes('--codex')) agentFlags.push('codex');
