...
`;function m(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}function j(e,t){switch(e){case"idle":return"Ready";case"pending":return"Waiting for agent...";case"fixing":return k[t%k.length];case"success":return"Done!";case"failed":return"Failed";default:return e}}function pe(e,t,n,i){if(e&&(!t||n)){let s=i;return s||(s=document.createElement("style"),s.id="eyeglass-cursor-style",document.head.appendChild(s)),s.textContent=`
      html, body, body * {
        cursor: ${ue} !important;
      }
    `,s}else return i&&(i.textContent=""),i}function _(){return`eyeglass-${Date.now()}-${Math.random().toString(36).slice(2,9)}`}function ge(e){let t=new EventSource(`${E}/events`);return t.onmessage=n=>{try{let i=JSON.parse(n.data);i.type==="activity"&&e.onActivityEvent(i.payload)}catch{}},t.onerror=()=>{t.close(),setTimeout(()=>e.onReconnect(),3e3)},t}async function Y(e){let t={interactionId:e.interactionId,userNote:e.userNote.trim(),autoCommit:e.autoCommit,...e.snapshots.length===1?{snapshot:e.snapshots[0]}:{snapshots:e.snapshots}};try{let n=await fetch(`${E}/focus`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});return n.ok?{success:!0}:{success:!1,error:`HTTP ${n.status}`}}catch{return{success:!1,error:"Failed to connect to bridge"}}}async function me(e,t,n,i){let o={interactionId:e,questionId:t,answerId:n,answerLabel:i};try{await fetch(`${E}/answer`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(o)})}catch{}}async function fe(e){try{return{success:(await fetch(`${E}/undo`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({interactionId:e})})).ok}}catch(t){return console.warn("Undo request failed:",t),{success:!1}}}async function be(e){try{return(await fetch(`${E}/commit`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({interactionId:e})})).ok?{success:!0}:(console.warn("Commit request failed"),{success:!1})}catch(t){return console.warn("Commit request failed:",t),{success:!1}}}function U(e,t,n){let i=n.map(s=>s.framework.componentName||s.tagName),o=n.length===1?i[0]:`${i.length} elements`;return{interactionId:e,userNote:t.trim(),componentName:o,filePath:n[0]?.framework.filePath,status:"pending",timestamp:Date.now()}}var ve=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
  <defs>
  ...
  `;let a=e.querySelector(".hub-header"),l=e.querySelector(".hub-disable"),d=e.querySelector(".hub-settings-btn");a.addEventListener("click",c=>{c.target===l||c.target===d||c.target.closest(".hub-settings-btn")||n.onToggleExpanded()}),d.addEventListener("click",c=>{c.stopPropagation(),n.onOpenSettings()}),l.addEventListener("click",c=>{c.stopPropagation(),n.onToggleEnabled()}),e.querySelectorAll(".hub-item-undo").forEach(c=>{c.addEventListener("click",p=>{p.stopPropagation();let g=p.currentTarget.dataset.id;n.onUndo(g)})})}function ye(e,t,n){e.className="hub",e.innerHTML=`
    <div class="hub-header">
      <div class="hub-header-left">
        <div class="hub-logo">${ve}</div>
  ...
  `,e.querySelector(".hub-back-btn").addEventListener("click",()=>{n.onBack()}),e.querySelectorAll(".theme-btn").forEach(o=>{o.addEventListener("click",s=>{let r=s.currentTarget.dataset.theme;n.onThemeChange(r)})}),e.querySelectorAll(".toggle-switch").forEach(o=>{o.addEventListener("click",s=>{s.currentTarget.dataset.setting==="autoCommit"&&n.onAutoCommitToggle()})})}function Se(e,t){let n=[];for(let i of e)if(i.type==="status"){if(i.status==="pending"||i.status==="fixing"&&(!i.message||i.message==="Agent is working..."))continue;n.push(_t(i))}else i.type==="thought"?n.push(Yt(i)):i.type==="action"?n.push(Ut(i)):i.type==="question"&&n.push(Kt(i,e));return n.length===0&&(t==="pending"||t==="fixing")?`
  ...
  `}function Ut(e){let t={reading:"\u{1F4D6}",writing:"\u270F\uFE0F",searching:"\u{1F50D}",thinking:"\u{1F9E0}"},n={reading:"Reading",writing:"Writing",searching:"Searching",thinking:"Thinking about"};return`
    <div class="activity-item">
  ...
  `}function we(e,t,n){if(n)return n;let i=window.innerHeight-e.bottom,o=t==="activity"?400:200,s=e.bottom+12;i<o&&e.top>o&&(s=e.top-o-12);let r=20,a=window.innerHeight-o-20;s=Math.max(r,Math.min(s,a));let l=e.left;return l+340>window.innerWidth-20&&(l=window.innerWidth-360),l<20&&(l=20),{x:l,y:s}}function Ee(e,t,n){let i=t.multiSelectMode,o=i?"multi-select-icon active":"multi-select-icon",s=i?`
    <div class="selected-list">
      ...
      <div class="selected-chips">
        ${t.selectedSnapshots.map((u,h)=>{let v=u.framework.componentName||u.tagName;return`
            <div class="selected-chip" data-index="${h}">
              <span class="selected-chip-number">${h+1}</span>
              <span>${m(v)}</span>
              <button class="selected-chip-remove" data-index="${h}" title="Remove">&times;</button>
            </div>
          `}).join("")}
      </div>
    </div>
  ...
  `;let a=e.querySelector(".input-field"),l=e.querySelector(".close-btn"),d=e.querySelector(".btn-secondary"),c=e.querySelector(".btn-primary"),p=e.querySelector(".multi-select-icon");l.addEventListener("click",()=>n.onClose()),d.addEventListener("click",()=>n.onClose()),c.addEventListener("click",()=>n.onSubmit(a.value)),c.addEventListener("keydown",u=>{(u.key==="Enter"||u.key===" ")&&(u.preventDefault(),n.onSubmit(a.value))}),a.addEventListener("keydown",u=>{u.key==="Enter"&&!u.shiftKey&&a.value.trim()&&(u.preventDefault(),n.onSubmit(a.value))}),p.addEventListener("click",()=>{n.onToggleMultiSelect()}),e.querySelectorAll(".selected-chip-remove").forEach(u=>{u.addEventListener("click",h=>{h.stopPropagation();let v=parseInt(h.currentTarget.dataset.index,10);n.onRemoveFromSelection(v)})}),e.querySelector(".panel-header").addEventListener("mousedown",n.onPanelDragStart),requestAnimationFrame(()=>a.focus())}function ke(e,t,n){let i=t.currentStatus==="success"||t.currentStatus==="failed",o=t.currentStatus==="success"&&!t.autoCommitEnabled,s=t.currentStatus==="success",r=t.submittedSnapshots.length,a=r>1?`${r} elements`:`&lt;${m(t.componentName)} /&gt;`,l=j(t.currentStatus,t.phraseIndex);e.innerHTML=`
    ...
    `:""}
  `,e.querySelector(".close-btn").addEventListener("click",()=>n.onClose()),e.querySelector(".panel-header").addEventListener("mousedown",n.onPanelDragStart),e.querySelectorAll(".question-option").forEach(x=>{x.addEventListener("click",Q=>{Q.stopPropagation();let z=Q.currentTarget,Ze=z.dataset.questionId,et=z.dataset.answerId,tt=z.textContent;n.onSubmitAnswer(Ze,et,tt)})});let p=e.querySelector(".action-btn-commit"),g=e.querySelector(".action-btn-undo");p&&p.addEventListener("click",()=>n.onCommit()),g&&g.addEventListener("click",()=>n.onUndo());let u=e.querySelector(".followup-input"),h=e.querySelector(".followup-send"),v=e.querySelector(".followup-done");u&&h&&(h.addEventListener("click",()=>{u.value.trim()&&n.onSubmitFollowUp(u.value)}),u.addEventListener("keydown",x=>{x.key==="Enter"&&!x.shiftKey&&u.value.trim()&&(x.preventDefault(),n.onSubmitFollowUp(u.value))}),requestAnimationFrame(()=>u.focus())),v&&v.addEventListener("click",()=>n.onClose());let b=e.querySelector(".activity-feed");b&&(b.scrollTop=b.scrollHeight)}function Ce(e,t,n){if(!e)return;let i=e.querySelector(".status-text");i&&(i.textContent=j(t,n))}function K(e){let t=[];e.perception?.legibility?.wcagStatus==="fail"&&t.push({level:"critical",category:"accessibility",message:"Low contrast ratio",details:`Contrast ratio: ${e.perception.legibility.contrastRatio}:1 (WCAG requires 4.5:1)`}),e.a11y?.hidden&&e.perception?.affordance?.isInteractable&&t.push({level:"critical",category:"accessibility",message:"Hidden but interactive",details:"Element is marked as hidden but has event listeners"});let n=e.metal?.performance?.lastRenderReason;if(n?.includes("'style' changed identity")&&t.push({level:"warning",category:"performance",message:"Inline style causing re-renders",details:n}),e.metal?.pipeline?.layoutThrashingRisk==="high"&&t.push({level:"warning",category:"performance",message:"Layout thrashing risk",details:"Element has listeners that may cause frequent layout recalculations"}),e.perception?.usability?.isTouchTargetValid===!1){let o=e.perception.usability.touchTargetSize;e.perception?.affordance?.isInteractable&&t.push({level:"warning",category:"usability",message:"Touch target too small",details:`Size: ${o} (should be at least 44x44)`})}e.perception?.affordance?.dissonanceScore&&e.perception.affordance.dissonanceScore>.5&&t.push({level:"warning",category:"usability",message:"Affordance mismatch",details:e.perception.affordance.looksInteractable?"Element looks clickable but has no interaction":"Element is interactive but does not look clickable"}),e.perception?.visibility?.isOccluded&&t.push({level:"warning",category:"usability",message:"Element is occluded",details:`Covered by: ${e.perception.visibility.occludedBy}`});let i=e.causality?.events?.blockingHandlers||[];if(i.length>0){let o=i.find(r=>r.reason==="pointer-events:none"),s=i.filter(r=>r.reason==="captured");o&&t.push({level:"critical",category:"events",message:"Events blocked",details:`pointer-events: none on ${o.element}`}),s.length>0&&t.push({level:"warning",category:"events",message:"Events may be captured",details:`${s.length} ancestor(s) using capture phase`})}return t}function L(e){let t=K(e);return t.some(n=>n.level==="critical")?"critical":t.some(n=>n.level==="warning")?"warning":"healthy"}function I(e){switch(e){case"critical":return"#ef4444";case"warning":return"#f59e0b";case"healthy":return"#10b981"}}function Me(e){let t=document.createElement("div");return t.className="loupe",t.setAttribute("aria-hidden","true"),e.appendChild(t),t}function Gt(e){let t=e.framework,n=t.displayName||t.componentName;if(n)return`<${n} />`;let i=`<${e.tagName}>`;return e.id?i=`#${e.id}`:e.className&&(i=`.${e.className.split(/\s+/)[0]}`),i}function Xt(e,t,n){let i=window.innerWidth,o=window.innerHeight,s=8,r=10,a=e.top-s,l=o-e.bottom-s,d=e.left-s,c=i-e.right-s,p,g,u;return a>=n+r?(p="above",g=e.left+(e.width-t)/2,u=e.top-n-r):l>=n+r?(p="below",g=e.left+(e.width-t)/2,u=e.bottom+r):c>=t+r?(p="right",g=e.right+r,u=e.top+(e.height-n)/2):d>=t+r?(p="left",g=e.left-t-r,u=e.top+(e.height-n)/2):(p="above",g=e.left+(e.width-t)/2,u=Math.max(s,e.top-n-r)),g=Math.max(s,Math.min(g,i-t-s)),u=Math.max(s,Math.min(u,o-n-s)),{position:p,x:g,y:u}}function He(e,t,n){let i=Gt(t),o=L(t),s=I(o),r=o!=="healthy";e.innerHTML=`
    <span class="loupe-name">${Vt(i)}</span>
    ${r?`<span class="loupe-pulse" style="background: ${s};"></span>`:""}
  `;let a=Math.min(300,i.length*8+30),l=28,{x:d,y:c}=Xt(n,a,l);e.style.transform=`translate(${Math.round(d)}px, ${Math.round(c)}px)`}function Te(e){e.classList.add("visible")}function G(e){e.classList.remove("visible")}function Vt(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}function Le(e){let t=e.framework;return t.displayName||t.componentName||e.tagName}function Wt(e){let t=e.framework;if(!t.filePath)return null;let i=t.filePath.split("/").slice(-2).join("/");return t.lineNumber?`${i}:${t.lineNumber}`:i}function Jt(e){return e.length===0?"":`
  ...
  `}function Ie(e,t){let{currentSnapshot:n,mode:i,multiSelectMode:o,selectedSnapshots:s}=e;if(o&&s.length>0)return Zt(e);if(!n)return'<div class="lens-empty">No element selected</div>';let r=Le(n),a=Wt(n),l=L(n),d=I(l),c=K(n);return i==="activity"?Qt(e,r,a):`
    <div class="lens-bar">
  ...
  `}function Qt(e,t,n){let{currentStatus:i,currentStatusMessage:o,activityEvents:s,_userNote:r}=e,a=[...s].reverse().find(p=>p.type==="thought"),l=[...s].reverse().find(p=>p.type==="action"),d=en(s,i),c="Working...";if(a)c=a.content;else if(l){let p=l;c=`${p.action}: ${p.target}${p.complete?" \u2713":"..."}`}else o?c=o:i==="pending"?c="Waiting for agent...":i==="success"?c="Done":i==="failed"&&(c="Failed");return`
    <div class="lens-bar">
    ...
    <div class="lens-activity">
      ...
      <div class="lens-message">${f(c)}</div>
    ...
    <div class="lens-selection">
      ${t.map((i,o)=>`
        <div class="lens-chip">
          ...
          <span class="chip-name">&lt;${f(Le(i))} /&gt;</span>
  ...
  `}function en(e,t){if(e.length===0)return"";let n=e.map(i=>tn(i)).filter(i=>!!i).join("");return n?`<div class="lens-feed">${n}</div>`:t==="pending"||t==="fixing"?`
        <div class="lens-feed">
      ...
      `:""}function tn(e){switch(e.type){case"status":return nn(e);case"thought":return`
      ...
      `;case"question":return sn(e);default:return null}}function nn(e){return e.status==="pending"||e.status==="fixing"&&(!e.message||e.message==="Agent is working...")?null:`
    <div class="lens-feed-item">
      <div class="lens-feed-dot status ${e.status}" title="${e.status}"></div>
      <div class="lens-feed-body">${f(e.message||e.status)}</div>
    </div>
  `}function sn(e){let t=!!e.selectedAnswerId,n=e.selectedAnswerLabel||e.options.find(i=>i.id===e.selectedAnswerId)?.label||"";return t?`
...
`;var P={component:"#3b82f6","state-owner":"#8b5cf6","layout-parent":"#f97316","event-blocker":"#ef4444"};function Ae(e,t){let n=[];n.push({element:e,type:"component",color:P.component,label:t.framework.displayName||t.framework.componentName||t.tagName});let i=e.parentElement;for(;i&&i!==document.body;){let o=getComputedStyle(i);if(o.display.includes("flex")||o.display.includes("grid")){n.push({element:i,type:"layout-parent",color:P["layout-parent"],label:`Layout (${o.display})`});break}i=i.parentElement}if(t.causality?.events?.blockingHandlers)for(let o of t.causality.events.blockingHandlers)try{let s=document.querySelector(o.element);s&&!n.some(r=>r.element===s)&&n.push({element:s,type:"event-blocker",color:P["event-blocker"],label:`Blocks ${o.event} (${o.reason})`})}catch{}if(t.framework.ancestry&&t.framework.ancestry.length>1){let o=Object.keys(e).find(s=>s.startsWith("__reactFiber$"));if(o){let s=e[o],r=0;for(;s&&r<10;){if(s.return&&s.return.type&&typeof s.return.type=="function"){let a=s.return.type.displayName||s.return.type.name;if(a&&s.return.stateNode instanceof Element){n.push({element:s.return.stateNode,type:"state-owner",color:P["state-owner"],label:`<${a} /> (props source)`});break}}s=s.return,r++}}}return n}function Ne(e,t){let n=[];for(let i of t){let o=i.element.getBoundingClientRect();if(o.width===0||o.height===0)continue;let s=document.createElement("div");s.className=`context-overlay context-${i.type}`,s.style.cssText=`
    ...
    `,this.toast.querySelector(".toast-close").addEventListener("click",()=>this.hideToast()),this.shadow.appendChild(this.toast),setTimeout(()=>this.hideToast(),4e3)}hideToast(){this.toast&&(this.toast.remove(),this.toast=null)}applyTheme(){this.setAttribute("data-theme",this.themePreference)}toggleInspectorEnabled(){this.inspectorEnabled=!this.inspectorEnabled,Ke(this.inspectorEnabled),this.inspectorEnabled||(this.unfreeze(),this.toggleCrosshair(!1)),this.updateCursor(),this.renderHub()}updateCursor(){this.cursorStyleElement=pe(this.inspectorEnabled,this.frozen,this.multiSelectMode,this.cursorStyleElement)}showHighlight(n){if(!this.highlight)return;let i=n.getBoundingClientRect(),o=3;if(this.highlight.style.display="block",this.highlight.style.left=`${i.left-o}px`,this.highlight.style.top=`${i.top-o}px`,this.highlight.style.width=`${i.width+o*2}px`,this.highlight.style.height=`${i.height+o*2}px`,!this.frozen&&this.loupe&&this.uiMode==="loupe"){let s=w(n);this.currentSnapshot=s,He(this.loupe,s,i),Te(this.loupe)}this.toggleCrosshair(!0)}updateCrosshair(n,i){if(!(!this.crosshairX||!this.crosshairY)){if(!this.inspectorEnabled){this.toggleCrosshair(!1);return}this.crosshairX.style.transform=`translateY(${i}px)`,this.crosshairY.style.transform=`translateX(${n}px)`,this.toggleCrosshair(!0)}}submitFromLensShortcut(){if(!this.lens)return;let n=this.lens.querySelector(".lens-input");if(!n)return;let i=n.value.trim();i&&this.submit(i)}toggleCrosshair(n){if(!this.crosshairX||!this.crosshairY)return;let i=n?"1":"0";this.crosshairX.style.opacity=i,this.crosshairY.style.opacity=i}hideHighlight(){this.highlight&&(this.highlight.style.display="none"),this.loupe&&!this.frozen&&G(this.loupe),this.toggleCrosshair(!1)}freeze(){this.currentElement&&(this.frozen=!0,this._userNote="",this.currentSnapshot=w(this.currentElement),this.selectedElements=[this.currentElement],this.selectedSnapshots=[this.currentSnapshot],this.mode="input",this.activityEvents=[],this.currentStatus="idle",this.updateCursor(),this.toggleCrosshair(!1),this.loupe&&G(this.loupe),this.uiMode="lens",this.renderLens())}unfreeze(){this.frozen=!1,this.currentSnapshot=null,this.interactionId=null,this.mode="input",this._userNote="",this.activityEvents=[],this.currentStatusMessage=null,this.customPanelPosition=null,this.customLensPosition=null,this.multiSelectMode=!1,this.selectedElements=[],this.selectedSnapshots=[],this.submittedSnapshots=[],A({multiSelectHighlights:this.multiSelectHighlights},this.shadow),this.multiSelectHighlights=[],this.stopPhraseRotation(),this.hideLens(),this.uiMode="loupe",this.hideContextOverlays(),this.hidePanel(),this.hideHighlight(),this.toggleCrosshair(!1),this.updateCursor(),X()}enterMultiSelectMode(){!this.frozen||this.multiSelectMode||(this.multiSelectMode=!0,this.multiSelectHighlights=M({multiSelectMode:!0,selectedElements:this.selectedElements,selectedSnapshots:this.selectedSnapshots,multiSelectHighlights:this.multiSelectHighlights,currentElement:this.currentElement,currentSnapshot:this.currentSnapshot},this.shadow),this.highlight&&(this.highlight.style.display="none"),this.updateCursor(),this.uiMode==="lens"?this.renderLens():this.renderPanel())}exitMultiSelectMode(){this.multiSelectMode=!1,this.selectedElements.length>0&&(this.currentElement=this.selectedElements[0],this.currentSnapshot=this.selectedSnapshots[0]),this.selectedElements=this.currentElement?[this.currentElement]:[],this.selectedSnapshots=this.currentSnapshot?[this.currentSnapshot]:[],A({multiSelectHighlights:this.multiSelectHighlights},this.shadow),this.multiSelectHighlights=[],this.currentElement&&this.showHighlight(this.currentElement),this.updateCursor(),this.uiMode==="lens"?this.renderLens():this.renderPanel()}toggleInSelection(n){if(!this.multiSelectMode)return;let i=this.selectedElements.indexOf(n);if(i>=0)this.removeFromSelection(i);else{let o=w(n);this.selectedElements.push(n),this.selectedSnapshots.push(o)}this.multiSelectHighlights=M({multiSelectMode:!0,selectedElements:this.selectedElements,selectedSnapshots:this.selectedSnapshots,multiSelectHighlights:this.multiSelectHighlights,currentElement:this.currentElement,currentSnapshot:this.currentSnapshot},this.shadow),this.highlight&&(this.highlight.style.display="none"),this.toggleCrosshair(!1),this.uiMode==="lens"?this.renderLens():this.renderPanel()}removeFromSelection(n){if(!(n<0||n>=this.selectedElements.length)){if(this.selectedElements.length===1){this.exitMultiSelectMode();return}this.selectedElements.splice(n,1),this.selectedSnapshots.splice(n,1),this.multiSelectHighlights=M({multiSelectMode:!0,selectedElements:this.selectedElements,selectedSnapshots:this.selectedSnapshots,multiSelectHighlights:this.multiSelectHighlights,currentElement:this.currentElement,currentSnapshot:this.currentSnapshot},this.shadow),this.highlight&&(this.highlight.style.display="none"),this.uiMode==="lens"?this.renderLens():this.renderPanel()}}renderHub(){this.hub||(this.hub=document.createElement("div"),this.hub.className="hub",this.shadow.appendChild(this.hub)),this.hubPage==="settings"?ye(this.hub,{themePreference:this.themePreference,autoCommitEnabled:this.autoCommitEnabled},{onBack:()=>{this.hubPage="main",this.renderHub()},onThemeChange:n=>{this.themePreference=n,J(n),this.applyTheme(),this.renderHub()},onAutoCommitToggle:()=>{this.autoCommitEnabled=!this.autoCommitEnabled,W(this.autoCommitEnabled),this.renderHub()}}):xe(this.hub,{hubExpanded:this.hubExpanded,inspectorEnabled:this.inspectorEnabled,history:this.history},{onToggleExpanded:()=>{this.hubExpanded=!this.hubExpanded,this.renderHub()},onToggleEnabled:()=>this.toggleInspectorEnabled(),onOpenSettings:()=>{this.hubPage="settings",this.hubExpanded=!0,this.renderHub()},onUndo:n=>this.handleUndoRequest(n)})}renderPanel(){if(!this.currentSnapshot||!this.currentElement)return;let n=this.currentElement.getBoundingClientRect(),{framework:i}=this.currentSnapshot;this.panel||(this.panel=document.createElement("div"),this.panel.className="glass-panel",this.shadow.appendChild(this.panel));let o=we(n,this.mode,this.customPanelPosition);this.panel.style.left=`${o.x}px`,this.panel.style.top=`${o.y}px`;let s=i.componentName||this.currentSnapshot.tagName,r=i.filePath?i.filePath.split("/").slice(-2).join("/"):null;this.mode==="input"?Ee(this.panel,{componentName:s,filePath:r,multiSelectMode:this.multiSelectMode,selectedSnapshots:this.selectedSnapshots},{onClose:()=>this.unfreeze(),onSubmit:a=>this.submit(a),onToggleMultiSelect:()=>{this.multiSelectMode?this.exitMultiSelectMode():this.enterMultiSelectMode()},onRemoveFromSelection:a=>this.removeFromSelection(a),onPanelDragStart:this.dragHandlers.handlePanelDragStart}):ke(this.panel,{componentName:s,filePath:r,submittedSnapshots:this.submittedSnapshots,activityEvents:this.activityEvents,currentStatus:this.currentStatus,autoCommitEnabled:this.autoCommitEnabled,userNote:this._userNote,interactionId:this.interactionId,phraseIndex:this.phraseIndex},{onClose:()=>this.unfreeze(),onSubmitFollowUp:a=>this.submitFollowUp(a),onSubmitAnswer:(a,l,d)=>this.handleSubmitAnswer(a,l,d),onCommit:()=>this.handleCommitRequest(),onUndo:()=>this.handleUndoFromPanel(),onPanelDragStart:this.dragHandlers.handlePanelDragStart})}hidePanel(){this.panel&&(this.panel.remove(),this.panel=null)}renderLens(){if(!this.currentSnapshot||!this.currentElement)return;this.lens||(this.lens=document.createElement("div"),this.lens.className="lens-card",this.shadow.appendChild(this.lens));let n={shadow:this.shadow,highlight:this.highlight,panel:this.panel,toast:this.toast,hub:this.hub,currentElement:this.currentElement,currentSnapshot:this.currentSnapshot,interactionId:this.interactionId,frozen:this.frozen,mode:this.mode,activityEvents:this.activityEvents,currentStatus:this.currentStatus,currentStatusMessage:this.currentStatusMessage,hubExpanded:this.hubExpanded,hubPage:this.hubPage,inspectorEnabled:this.inspectorEnabled,autoCommitEnabled:this.autoCommitEnabled,themePreference:this.themePreference,history:this.history,isDragging:this.isDragging,dragOffset:this.dragOffset,customPanelPosition:this.customPanelPosition,multiSelectMode:this.multiSelectMode,selectedElements:this.selectedElements,selectedSnapshots:this.selectedSnapshots,multiSelectHighlights:this.multiSelectHighlights,submittedSnapshots:this.submittedSnapshots,cursorStyleElement:this.cursorStyleElement,throttleTimeout:this.throttleTimeout,scrollTimeout:this.scrollTimeout,phraseIndex:this.phraseIndex,phraseInterval:this.phraseInterval,_userNote:this._userNote,eventSource:this.eventSource},i={unfreeze:()=>this.unfreeze(),submit:a=>this.submit(a),submitFollowUp:a=>this.submitFollowUp(a),submitAnswer:(a,l,d)=>this.handleSubmitAnswer(a,l,d),requestUndo:a=>this.handleUndoRequest(a),requestCommit:a=>this.handleCommitRequest(),enterMultiSelectMode:()=>this.enterMultiSelectMode(),exitMultiSelectMode:()=>this.exitMultiSelectMode(),removeFromSelection:a=>this.removeFromSelection(a),toggleHubExpanded:()=>{this.hubExpanded=!this.hubExpanded,this.renderHub()},toggleInspectorEnabled:()=>this.toggleInspectorEnabled(),openSettingsPage:()=>{this.hubPage="settings",this.hubExpanded=!0,this.renderHub()},closeSettingsPage:()=>{this.hubPage="main",this.renderHub()},setTheme:a=>{this.themePreference=a,J(a),this.applyTheme()},toggleAutoCommit:()=>{this.autoCommitEnabled=!this.autoCommitEnabled,W(this.autoCommitEnabled)},handlePanelDragStart:this.dragHandlers.handlePanelDragStart,renderHub:()=>this.renderHub(),renderPanel:()=>this.renderPanel()};this.lens.innerHTML=Ie(n,i);let o=this.currentElement.getBoundingClientRect(),s={width:220,height:this.lens.offsetHeight||200},r=this.customLensPosition??Pe(o,s);this.lens.style.left=`${r.x}px`,this.lens.style.top=`${r.y}px`,this.wireLensEvents()}wireLensEvents(){if(!this.lens)return;let n=this.lens.querySelector(".lens-bar");n&&n.addEventListener("mousedown",this.dragHandlers.handleLensDragStart);let i=this.lens.querySelector('[data-action="toggle-schema"]');i&&i.addEventListener("click",()=>this.toggleSchemaView());let o=this.lens.querySelector('[data-action="close"]');o&&o.addEventListener("click",()=>this.unfreeze());let s=this.lens.querySelector(".lens-input");s&&(s.addEventListener("keydown",h=>{h.key==="Enter"&&!h.shiftKey&&s.value.trim()&&(this.submit(s.value),h.preventDefault())}),setTimeout(()=>s.focus(),100));let r=this.lens.querySelector(".lens-enter-btn");r&&s&&r.addEventListener("click",()=>{s.value.trim()?this.submit(s.value):s.focus()});let a=this.lens.querySelector('[data-action="multi-select"]');a&&a.addEventListener("click",()=>{this.multiSelectMode?this.exitMultiSelectMode():this.enterMultiSelectMode(),this.renderLens()});let l=this.lens.querySelector('[data-action="toggle-context"]');l&&l.addEventListener("click",()=>this.toggleContextOverlays());let d=this.lens.querySelector('[data-action="peek-schema"]');d&&d.addEventListener("click",()=>this.toggleSchemaView());let c=this.lens.querySelector('[data-action="exit-multi"]');c&&c.addEventListener("click",()=>{this.exitMultiSelectMode(),this.renderLens()}),this.lens.querySelectorAll('[data-action="remove-selection"]').forEach(h=>{h.addEventListener("click",()=>{let v=parseInt(h.getAttribute("data-index")||"0",10);this.removeFromSelection(v),this.renderLens()})}),this.lens.querySelectorAll('[data-action="answer"]').forEach(h=>{h.addEventListener("click",()=>{let v=h.getAttribute("data-question-id")||"",b=h.getAttribute("data-answer-id")||"",x=h.getAttribute("data-answer-label")||"";this.handleSubmitAnswer(v,b,x)})}),this.lens.querySelectorAll('[data-action="issue-insert"]').forEach(h=>{h.addEventListener("click",()=>{let v=h.getAttribute("data-issue")||"";if(!v)return;let b=this.lens?.querySelector(".lens-input");if(!b)return;let x=b.value.length>0&&!b.value.endsWith(`
`);b.value=`${b.value}${x?`
`:""}${v}`,this._userNote=b.value,b.dispatchEvent(new Event("input",{bubbles:!0})),b.focus(),b.setSelectionRange(b.value.length,b.value.length)})});let p=this.lens.querySelector('[data-action="new-request"]');p&&p.addEventListener("click",()=>{this.mode="input",this._userNote="",this.activityEvents=[],this.currentStatus="idle",this.renderLens()});let g=this.lens.querySelector(".lens-followup-send"),u=this.lens.querySelector(".lens-followup-input");g&&u&&(g.addEventListener("click",()=>{let h=u.value.trim();h&&this.submitFollowUp(h)}),u.addEventListener("keydown",h=>{if(h.key==="Enter"&&!h.shiftKey){h.preventDefault();let v=u.value.trim();v&&this.submitFollowUp(v)}}))}hideLens(){this.lens&&(this.lens.remove(),this.lens=null)}toggleContextOverlays(){this.showingContextOverlays?this.hideContextOverlays():this.showContextOverlays()}showContextOverlays(){!this.currentElement||!this.currentSnapshot||(this.showingContextOverlays=!0,this.contextOverlays=Ae(this.currentElement,this.currentSnapshot),this.contextOverlayElements=Ne(this.shadow,this.contextOverlays))}hideContextOverlays(){this.showingContextOverlays=!1,ze(this.shadow),this.contextOverlays=[],this.contextOverlayElements=[]}toggleSchemaView(){if(!this.lens||!this.currentSnapshot)return;let n=this.lens.querySelector(".lens-schema"),i=this.lens.querySelector(".lens-schema-code");if(!n||!i)return;if(n.getAttribute("data-expanded")==="true")n.setAttribute("data-expanded","false");else{let s=JSON.stringify(this.currentSnapshot,null,2);i.innerHTML=this.highlightJson(s),n.setAttribute("data-expanded","true")}}highlightJson(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/("(?:\\.|[^"\\])*")\s*:/g,'<span class="json-key">$1</span>:').replace(/:\s*("(?:\\.|[^"\\])*")/g,': <span class="json-string">$1</span>').replace(/:\s*(\d+\.?\d*)/g,': <span class="json-number">$1</span>').replace(/:\s*(true|false)/g,': <span class="json-bool">$1</span>').replace(/:\s*(null)/g,': <span class="json-null">$1</span>').replace(/([{}\[\]])/g,'<span class="json-bracket">$1</span>')}startPhraseRotation(){this.phraseInterval||(this.phraseIndex=Math.floor(Math.random()*k.length),this.phraseInterval=window.setInterval(()=>{this.phraseIndex=(this.phraseIndex+1)%k.length,Ce(this.panel,this.currentStatus,this.phraseIndex)},1e4))}stopPhraseRotation(){this.phraseInterval&&(window.clearInterval(this.phraseInterval),this.phraseInterval=null)}async submit(n){if(!n.trim()||this.selectedSnapshots.length===0&&!this.currentSnapshot)return;this.interactionId=_(),this._userNote=n.trim();let i=this.selectedSnapshots.length>0?this.selectedSnapshots:this.currentSnapshot?[this.currentSnapshot]:[];this.submittedSnapshots=[...i];let o=U(this.interactionId,n,i);this.history=V(this.history,o),y(this.history),this.renderHub();let s=this.multiSelectMode,r=[...this.selectedElements],a=[...this.selectedSnapshots];A({multiSelectHighlights:this.multiSelectHighlights},this.shadow),this.multiSelectHighlights=[],this.multiSelectMode=!1,this.mode="activity",this.activityEvents=[],this.currentStatus="pending",this.currentStatusMessage=null,this.uiMode==="lens"?this.renderLens():this.renderPanel();let l=await Y({interactionId:this.interactionId,userNote:n.trim(),autoCommit:this.autoCommitEnabled,snapshots:i});l.success||(this.currentStatus="failed",this.history=$(this.history,this.interactionId,"failed"),y(this.history),this.renderHub(),this.activityEvents.push({type:"status",interactionId:this.interactionId,status:"failed",message:l.error||"Failed to connect to bridge",timestamp:Date.now()}),s&&r.length>1&&(this.multiSelectMode=!0,this.selectedElements=r,this.selectedSnapshots=a,this.mode="input",this.multiSelectHighlights=M({multiSelectMode:!0,selectedElements:this.selectedElements,selectedSnapshots:this.selectedSnapshots,multiSelectHighlights:this.multiSelectHighlights,currentElement:this.currentElement,currentSnapshot:this.currentSnapshot},this.shadow),this.highlight&&(this.highlight.style.display="none")),this.uiMode==="lens"?this.renderLens():this.renderPanel())}async submitFollowUp(n){if(!n.trim()||this.submittedSnapshots.length===0)return;this.interactionId=_(),this._userNote=n.trim();let i=this.submittedSnapshots,o=U(this.interactionId,n,i);this.history=V(this.history,o),y(this.history),this.renderHub(),this.activityEvents=[],this.currentStatus="pending",this.currentStatusMessage=null,this.uiMode==="lens"?this.renderLens():this.renderPanel();let s=await Y({interactionId:this.interactionId,userNote:n.trim(),autoCommit:this.autoCommitEnabled,snapshots:i});s.success||(this.currentStatus="failed",this.history=$(this.history,this.interactionId,"failed"),y(this.history),this.renderHub(),this.activityEvents.push({type:"status",interactionId:this.interactionId,status:"failed",message:s.error||"Failed to connect to bridge",timestamp:Date.now()}),this.uiMode==="lens"?this.renderLens():this.renderPanel())}async handleSubmitAnswer(n,i,o){if(!this.interactionId)return;let s=this.activityEvents.find(r=>r.type==="question"&&r.questionId===n);s&&(s.selectedAnswerId=i,s.selectedAnswerLabel=o),this.uiMode==="lens"?this.renderLens():this.renderPanel(),await me(this.interactionId,n,i,o)}async handleUndoRequest(n){let i=this.history.findIndex(s=>s.interactionId===n);if(i===-1)return;this.history[i].status="pending",y(this.history),this.renderHub(),(await fe(n)).success?this.history=Ye(this.history,n):this.history[i].status="failed",y(this.history),this.renderHub()}async handleUndoFromPanel(){this.interactionId&&await this.handleUndoRequest(this.interactionId)}async handleCommitRequest(){if(!this.interactionId)return;let n=this.history.findIndex(o=>o.interactionId===this.interactionId);(await be(this.interactionId)).success&&(n>=0&&(this.history[n].status="success",y(this.history),this.renderHub()),this.unfreeze())}};customElements.get("eyeglass-inspector")||customElements.define("eyeglass-inspector",N);function Qe(){if(document.querySelector("eyeglass-inspector")){console.warn("[eyeglass] Inspector already initialized");return}let e=document.createElement("eyeglass-inspector");document.body.appendChild(e),console.log("[eyeglass] Inspector initialized. Hover over elements and click to annotate.")}typeof window<"u"&&typeof document<"u"&&(typeof process<"u"||(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Qe):Qe()));export{N as EyeglassInspector,w as captureSnapshot,D as extractFrameworkInfo,Qe as initInspector};
