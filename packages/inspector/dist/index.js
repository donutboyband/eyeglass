  ...
  `}function tn(e){if(e.multiSelectMode)return"";let n=e.domPaused?"active":"";return`
    <div class="lens-state-toolbar">
      ...
      <div class="lens-state-actions">
        ...
        <button class="state-btn ${n}" data-action="toggle-pause" title="Pause DOM (\u2318/Ctrl + Shift + P)">${e.domPaused?"Resume DOM":"Pause DOM"}</button>
      </div>
    </div>
    ${nn(e)}
  `}function nn(e){return e.stateCapsules.length?`
    <div class="lens-capsules">
      ${e.stateCapsules.map(n=>`
        <div class="lens-capsule ${e.activeCapsuleId===n.id?"active":""}" data-action="select-capsule" data-capsule-id="${n.id}">
          <div class="capsule-name">&lt;${f(n.label)}&gt;</div>
          <div class="capsule-time">${new Date(n.capturedAt).toLocaleTimeString()}</div>
          <button class="capsule-remove" data-action="delete-capsule" data-capsule-id="${n.id}" title="Remove">\xD7</button>
        </div>
      `).join("")}
    </div>
